...
function updateHours(day) {
    ...
    if (checkIn && checkOut) {
        ...
        saveData(); // حفظ البيانات حتى عند ترك وقت الحضور أو الانصراف فارغًا
    }
}
...
function calculateHoursWorked(checkIn, checkOut) {
    ...
    let start = new Date();
    start.setHours(checkInHour, checkInMinute, 0, 0);

    let end = new Date();
    end.setHours(checkOutHour, checkOutMinute, 0, 0);

    if (end < start) {
        end.setDate(end.getDate() + 1); // إذا كان وقت الانصراف أقل من وقت الحضور، نضيف يومًا
    }
